...
// Connection ConnectionManager::fcon;

ConnectionManager::ConnectionManager() : sPortNameNow(UserDefine::NO_DEVICE_FOUND), fIsOpen(false)
{
    zaber::motion::Library::enableDeviceDbStore();
}

vector<Device> ConnectionManager::getDeviceList()
{
    return fDeviceList;
}
...
string ConnectionManager::ScanDevice()
{
    Init();
...
bool ConnectionManager::ScanPort(string portName)
{
    Init();
...
{
    static ConnectionManager *currentConMan = new ConnectionManager;
